...
export function NewConversationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await onStart(email);
      if (result.ok) {
        setEmail("");
        onClose();
      } else {
  ...
  return (
    ...
    >
      ...
      >
        <h2 id="new-conversation-title" className="text-lg font-semibold text-white mb-3">
          Nova conversa
        </h2>
        <p className="text-sm text-white/70 mb-4">
          Digite o e-mail de quem você quer conversar. Não precisa estar nos contatos.
        </p>
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2">
            ...
            >
              {loading ? "Buscando..." : "Iniciar conversa"}
            </button>
            <button
              type="button"
              onClick={handleClose}
